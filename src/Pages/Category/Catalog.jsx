    ...
    return (
        <Layout style={{ backgroundColor: "#fff" }}>
            <Content style={{ padding: "40px 24px", maxWidth: "1200px", margin: "0 auto" }}>
                <Title level={2}>Catálogo de Instrumentos</Title>
                ...
                {/* Lista de Categorías e Instrumentos */}
                {categorias.map((categoria) => (
                    <div key={categoria.id} style={{ marginBottom: "40px" }}>
                        <Title level={3}>{categoria.nombre}</Title>
                        <Row gutter={[24, 24]} justify="start">
                            {categoria.instrumentos.map((instrumento) => (
                                <Col key={instrumento.id} xs={24} sm={12} md={8} lg={6}>
                                    <Card 
                                        cover={<img src={instrumento.imagen} alt={instrumento.nombre} style={{ height: "200px", objectFit: "cover" }} />}
